...
osThreadId StateMachineTskHandle;
osThreadId VoltageSensTskHandle;
osThreadId PCurrentSensTskHandle;
osThreadId PumpControlTskHandle;   uint8_t cec_receive_buffer[16];
/* USER CODE BEGIN PV */
...
void VoltageSensorTask(void const * argument);
void PumpCurrentSensorTask(void const * argument);
void PumpControlTask(void const * argument);
...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  osThreadDef(PumpControlTsk, PumpControlTask, osPriorityNormal, 0, 512);
  PumpControlTskHandle = osThreadCreate(osThread(PumpControlTsk), NULL);
